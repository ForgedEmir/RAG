...
from src.api.auth import get_optional_user, require_monitoring
...
from src.monitoring.tracker import (
    count_user_exchanges, delete_conversation, get_conversation,
    get_history, get_user_summary, save_exchange, save_user_summary, track,
)
...
class AskBody(BaseModel):
    question: str
    session_id: str = ""
    user_id: Optional[str] = None  # ignoré si JWT présent
...
async def ask(request: Request, body: AskBody, user_id: str = Depends(get_optional_user)):
    req_id = str(uuid.uuid4())[:8]
    start  = time.time()

    user_id = user_id or body.user_id or ""
    ...
    if not user_id:
        return JSONResponse({"error": "user_id manquant (connectez-vous ou fournissez user_id)"}, status_code=400)
            ...
            if session_id:
                ...
                if _is_important(question, answer):
                    ...
                    if total > 0 and total % _SUMMARY_INTERVAL == 0:
                        threading.Thread(target=_run_summary, args=(user_id, history), daemon=True).start()
                    threading.Thread(target=add_user_memory, args=(user_id, question, answer), daemon=True).start()
...
async def del_conv(session_id: str = ""):
    if not session_id:
        return JSONResponse({"error": "session_id requis"}, status_code=400)
